...
import ru.blimfy.common.json.MessageReferenceReadConverter
import ru.blimfy.common.json.MessageReferenceWriteConverter
import ru.blimfy.common.json.MessageSnapshotReadConverter
import ru.blimfy.common.json.MessageSnapshotWriteConverter

/**
 * Авто-конфигурация для подключения бинов для работы с сущностями сервера в БД.
 *
...
@ComponentScan
@PropertySource("classpath:bm-server-db.yml", factory = CustomConfigPropertiesReaderFactory::class)
class ServerDatabaseAutoConfiguration {
    @Value("\${bm-server-db.liquibase.changelog}")
    ...
    /**
     * Создаёт бин Liquibase для выполнения скриптов миграции БД с серверами.
     */
    @Bean
    fun serverLiquibase() =
