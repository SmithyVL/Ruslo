...
/**
 * Контроллер для работы с каналами.
 *
 * @property service сервис для обработки информации о каналах.
 * @author Владислав Кузнецов.
 * @since 0.0.1.
 */
@Tag(name = "ChannelController", description = "Контроллер для работы с каналами")
@RestController
@RequestMapping("/v1/channels")
class ChannelController(private val service: ChannelControllerService) {
    @Operation(summary = "Получить канал")
    @GetMapping("/{id}")
    suspend fun findChannel(
        @PathVariable id: UUID,
        @AuthenticationPrincipal userDetails: CustomUserDetails,
    ) = service.findChannel(id, userDetails.info)
