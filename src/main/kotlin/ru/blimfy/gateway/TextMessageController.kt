...
import ru.blimfy.websockets.dto.WsMessageTypes.EDIT_TEXT_MESSAGE
import ru.blimfy.websockets.dto.WsMessageTypes.NEW_TEXT_MESSAGE
import ru.blimfy.websockets.dto.WsMessageTypes.REMOVE_TEXT_MESSAGE
...
class TextMessageController(
    ...
    suspend fun createTextMessage(
        ...
        return textMessageService.saveMessage(textMessageDto.toEntity(userId)).toDto()
            .apply { userWebSocketService.sendServerMessages(serverId, NEW_TEXT_MESSAGE, this, userId) }
    }
    ...
    suspend fun modifyTextMessage(
        ...
        return textMessageService.saveMessage(textMessageDto.toEntity(userId)).toDto()
            .apply { userWebSocketService.sendServerMessages(serverId, EDIT_TEXT_MESSAGE, this, userId) }
    }
    ...
    suspend fun deleteTextMessage(@PathVariable textMessageId: UUID, principal: Principal) {
        ...
        textMessageService.deleteMessage(id = textMessageId, authorId = userId)
            .apply { userWebSocketService.sendServerMessages(serverId, REMOVE_TEXT_MESSAGE, textMessageId, userId) }
    }
}